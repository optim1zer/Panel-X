...
//Работа с антигейтом
function antigate($filename, $apikey, $is_verbose = false, $rtimeout = 5, $mtimeout = 120, $is_phrase = 0, $is_regsense = 0, $is_numeric = 0, $min_len = 0, $max_len = 0) {
	...
	if ($userconfig['yandex_method'] == "SIMPLE") {
		...
		if (($sResp = urlGetContents($urlAddress)) === false) {
			...
			if (strpos($sResp, "http://yandex.ru/checkcaptcha") !== false) {
				preg_match("/<input name=\"key\" type=\"hidden\" value=\"(.*)\">/U", $sResp, $cpmatches);
				preg_match("/<input name=\"retpath\" type=\"hidden\" value=\"(.*)\">/U", $sResp, $pathmatches);
				preg_match("/src=\"(.*)\" class=\"b-captcha__image\"/U", $sResp, $imgmatches);
				$imgkey = time();
				$fp = fopen("writing/" . $imgkey . ".gif", "w");
				fwrite($fp, file_get_contents($imgmatches[1]));
				fclose($fp);
				//$key = capchabot("tmp/".$cpmatches[1].".gif");
				$key = antigate("writing/" . $imgkey . ".gif", $userconfig['antigate_key']);
				$sResp = urlGetContents("http://yandex.ru/checkcaptcha?key=" . $cpmatches[1] . "&retpath=" . $pathmatches[1] . "&rep=" . $key);
			}
			$sl1 = "(Нашлось|Нашёлся|Нашлась)"; //Нашёлся 1&nbsp;ответ
			$sl2 = "(ответов|ответ|ответа)";
			preg_match('/' . $sl1 . ' (\d+)&nbsp;' . $sl2 . '/', $sResp, $a);
			$num = (int) @$a[2];
			if ($num == 0) {
				$sl1 = "(Нашлось|Нашёлся|Нашлась)";
				$sl2 = "тыс.&nbsp;ответ";
				preg_match('/' . $sl1 . '.(\d+)&nbsp;' . $sl2 . '/', $sResp, $a);
				$num = ((int) @$a[2]) * 1000;
			}
...
function getDMOZ($url) {
	...
	if (preg_match('!Open Directory Sites!ism', $buf)) {
		return (1);
	} else {
		return (0);
	}
...
function getYACA($site) {
	$buf = urlGetContents('http://search.yaca.yandex.ru/yca/cy/ch/' . $site . '/', 80, 15);
	// нигде не встречается
	if (!preg_match('!не описан!ism', $buf)) {
		return(1);
	} else {
		return(0);
	}
...
function list_updates() {
	...
	if (!$last || (( $now - $last ) > $interval)) {
		$sResp = urlGetContents("http://pr-cy.ru/updates");
		$sResp = str_replace("\n", "", str_replace(" ", "", str_replace("\t", "", $sResp)));
		preg_match('/<tr><tdvalign="top">(\d+).(\d+).(\d+)<br\/>(.*)<\/td><tdvalign="top">(\d+).(\d+).(\d+)<br\/>(.*)<divid="cy">(.*)<\/td><tdvalign="top">(\d+).(\d+).(\d+)<br\/>(.*)<divid="se">/', $sResp, $a); //PR
		$content = "<b style=\"color:#646464;\">PR</b>: <span class=\"count\">" . $a[1] . "." . $a[2] . "." . $a[3] . "</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b style=\"color:#646464;\">тИЦ</b>: <span class=\"count\">" . $a[5] . "." . $a[6] . "." . $a[7] . "</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b style=\"color:#646464;\">ЯВ</b>: <span class=\"count\">" . $a[10] . "." . $a[11] . "." . $a[12] . "</span>";
		$fp = fopen("writing/updates.txt", "w");
		fwrite($fp, $content);
		fclose($fp);
	}
	$ret = file_get_contents("writing/updates.txt");
	return($ret);
}
