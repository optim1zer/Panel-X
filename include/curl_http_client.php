...
class Curl_HTTP_Client
{
	...
	/**
	 * Set proxyport to use for each curl request
	...
	/**
	 * Set proxy auth to use for each curl request
	 * @param string username:password
	 * @access public
	 */
	function set_proxyauth($proxyauth)
	{
		curl_setopt($this->ch, CURLOPT_PROXYAUTH, $proxyauth);
	}
	/**
	...
	function send_multipart_post_data($url, $postdata, $file_field_array=array(), $ip=null, $timeout=30)
	{
		...
		$headers = array('Transfer-Encoding: ','Expect:');
	...
	{
		if(strlen($url)>0) curl_setopt($this->ch, CURLOPT_URL, $url);
		return curl_getinfo($this->ch, CURLINFO_HTTP_CODE);
	}
	function get_response_only($url) {
		// set url to post to
		curl_setopt($this->ch, CURLOPT_URL,$url);
 ...
 */
	...
	function curl_exec_redir($ch)
	{
		...
		$latest_visited = curl_getinfo($ch, CURLINFO_EFFECTIVE_URL);

		curl_setopt($ch, CURLOPT_HTTPHEADER, array('Expect:'));
		curl_setopt($ch, CURLOPT_HEADER, true);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
		curl_setopt($ch, CURLOPT_VERBOSE, true);
		if(defined("COOKIEFILE") AND strlen(COOKIEFILE)>0) {
		...
		if ( $http_code == 301 || $http_code == 302 )
		{
			...
			$latest_visited = curl_getinfo($ch, CURLINFO_EFFECTIVE_URL);
			$last_url = parse_url($latest_visited);
			curl_setopt($ch, CURLOPT_REFERER, $latest_visited);
			if (!$url['scheme'])
			...
			$new_url = $url['scheme'] . '://' . $url['host'] . $url['path'] . (!empty($url['query'])?'?'.str_replace(array(" ","*"), array("%20","*"), $url['query']):'');
			if(strpos($new_url, "http://yandex.ru/yandsearch?text") !== false) $new_url .= "&lr=225";
