	...
	$.fn.quicksearch = function (opt) {
		...
		function loader(o) {
			if (options.loaderId)
			{
				...
				{
					l.html('<img src="images/loading_white.gif" border="0" alt="" style="vertical-align: middle;" /> Обработка запроса');
				}
			}
		}
		...
		function make_form_input()
		{
			...
			return '<input type="text" value="' + val + '" rel="' + options.randomElement  + '" id="' + options.randomElement + '" /> ';
		}
...
//				return '<span id="' + options.loaderId + '" class="' + options.loaderClass + '">' + options.loaderText + '</span>';
		...
		function make_form()
		{
			...
			return '<' + f + ' action="#" ' + 'id="'+ options.formId + '" ' + 'class="quicksearch">' +
						make_form_label() +	make_form_input() +
					'</' + f + '>';
		}
		...
		var options = $.extend({
			...
			focusOnLoad: false,
			onBefore: function () { },
			onAfter: function () { $("#tablesorter").trigger("update");$("#tablesorter").trigger("appendCache");row_highlight();make_options(); },
			filter: function (i) {
				return i;
			},
